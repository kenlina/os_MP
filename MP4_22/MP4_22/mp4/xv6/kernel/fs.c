...
bmap(struct inode *ip, uint bn)
{
  ...
  if(bn < NINDIRECT){
    ...
    int index = bn / (BSIZE / sizeof(uint)) + NDIRECT;
    int offset = bn % (BSIZE / sizeof(uint));
  ...
  if(bn < NDOUBLYINDIRECT){
    ...
    int outer = 12;
    int inner = bn / (BSIZE / sizeof(uint));
    int offset = bn % (BSIZE / sizeof(uint));
    // outer layer
    if((addr = ip->addrs[outer]) == 0)
      ip->addrs[outer] = addr = balloc(ip->dev);
    // inner layer
    bp = bread(ip->dev, addr);
    a = (uint*)bp->data;
    if((addr = a[inner]) == 0){
...
namex(char *path, int nameiparent, char *name)
{
  ...
  while((path = skipelem(path, name)) != 0){
    ...
    if((next = dirlookup(ip, name, 0)) == 0){
      // printf("namex: dirlookup failed can't find %s\n", name);
      // printf("cwd is %s\n");
      iunlockput(ip);
      depth = 0;
      return 0;
