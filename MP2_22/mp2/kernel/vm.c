...
void __vmprint__(pagetable_t pagetable){
    ...
    for(int out = 0; out < 512; ++out){
        if(pagetable[out] & PTE_V){
            pagetable_t pte_o = &pagetable[out];
            uint64 va_o = (uint64)out << PXSHIFT(2);
            pagetable_t pa_o = (pagetable_t)PTE2PA(pagetable[out]);
            printf("%s %d: pte=%p va=%p pa=%p V", (out == last_out_print) ? "└──" : "├──", out, pte_o, va_o, pa_o);
            ...
            for(int mid = 0; mid < 512; ++mid){
                if(pa_o[mid] & PTE_V){
                    pagetable_t pte_m = &pa_o[mid];
                    uint64 va_m = va_o | ((uint64)mid << PXSHIFT(1));
                    pagetable_t pa_m = (pagetable_t)PTE2PA(pa_o[mid]);
                    // const char* format_m = (mid == last_mid_print) ? (out == last_out_print ? "    └──" : "│   └──") : (out == last_out_print ? "    ├──" : "│   ├──");
                    printf("%s", (out == last_out_print) ? "    " : "│   ");
                    printf("%s", (mid == last_mid_print) ? "└──"  : "├──");
                    printf(" %d: pte=%p va=%p pa=%p V", mid, pte_m, va_m, pa_m);
                    test_flag(pa_o, mid);
                    int last_last_print = -1;
                    for (int temp_last = 511; temp_last >= 0; --temp_last) {
                        if ((pa_m[temp_last] & PTE_V) || (pa_m[temp_last] & PTE_S)){
                            last_last_print = temp_last;
                            break;
                        } 
                    }
                    ...
                    for(int last = 0; last < 512; ++last){
                        if(pa_m[last] & PTE_V){
                            pagetable_t pte_last = &pa_m[last];
                            uint64 va_last = va_m | ((uint64)last << PXSHIFT(0));
                            uint64 pa_last = PTE2PA(pa_m[last]);
                            printf("%s", (out == last_out_print) ? "    " : "│   ");
                            printf("%s", (mid == last_mid_print) ? "    " : "│   ");
                            printf("%s", (last == last_last_print) ? "└──" : "├──");
                            printf(" %d: pte=%p va=%p pa=%p V", last, pte_last, va_last, pa_last);
                            test_flag(pa_m, last);
                        }
                        if(pa_m[last] & PTE_S){
                            pagetable_t pte_last = &pa_m[last];
                            ...
                            printf("%s", (mid == last_mid_print) ? "    " : "│   ");
                            printf("%s", (last == last_last_print) ? "└──" : "├──");
                            printf(" %d: pte=%p va=%p blockno=%p", last, pte_last, va_last, blockno);
                            test_flag(pa_m, last);
                        }
                    }
                }
...
int madvise(uint64 base, uint64 len, int advice) {
    ...
    if(advice == MADV_DONTNEED){
        for(uint64 a = base; a < end; a += PGSIZE){
            ...
            if(*pte & PTE_V){
                ...
                write_page_to_disk(ROOTDEV, pa, blockno);
                end_op();
                kfree((void*)pa);
