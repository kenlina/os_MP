    ...
    *not* match any line in text."""
    ...
    for lineno in sorted(show):
        if 0 <= lineno < len(lines):
            ...
            msg.append(
                "%s %s" %
                (color("red", "BAD ") if lineno in bad else
                 color("green", "GOOD") if lineno in good else "    ", lines[lineno]))
    if last != len(lines) - 1:
        msg.append("...")
    if bad:
...
'killall qemu' or 'killall qemu.real'.""" % self.get_gdb_port(),
    ...
    def get_gdb_port():
        if QEMU._GDBPORT is None:
            ...
            (out, _) = p.communicate()
            if p.returncode:
                raise RuntimeError("Failed to get gdbport: make exited with %d" %
                                   p.returncode)
...
class Runner():
    ...
    def match(self, *args, **kwargs):
        ...
        for i, line in enumerate(lines):
            # print(i, lne, ans_idx)
            if ans_idx == -1:
                if '$ {}'.format(args[0].split()[0]) in line:
                    ans_idx += 1
                # else:
                # print('|{}| == |{}|'.format(args[0], line))
                continue

            if args[ans_idx] != line:
                msg = "[Error] Output on line {}: \'{}\', should be \'{}\'".format(
                    ans_idx, line, args[ans_idx])
        ...
        if ans_idx != len(args):
            if ans_idx < 0:
                ans_idx = 0
            msg = "[Error] Missing output on line {}: {}".format(ans_idx, args[ans_idx])
            raise AssertionError(msg)
